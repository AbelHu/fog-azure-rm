...
class TestCompareBlobs < Minitest::Test
  ...
  def test_compare_blobs_success
    @service.stub :get_identical_blobs_from_containers, @blob_list do
      assert_equal @blob_list, @service.compare_blob('test_container1', 'test_container2')
    end
  end

  def test_compare_blobs_http_exception
    http_exception = -> (*) { raise Azure::Core::Http::HTTPError.new(@mocked_response) }
    @service.stub :get_identical_blobs_from_containers, http_exception do
      assert_raises(RuntimeError) do
        @service.compare_blob('test_container1', 'test_container2')
      end
    end
  end

  def test_compare_blobs_mock
    assert_equal @blob_list, @mock_service.compare_blob('test_container1', 'test_container2')
  end
end
